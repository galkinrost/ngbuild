    ...
    astra(ast, true)
        .when([
            ...
            while (i--) {
                ...
                if (property.key.name === 'template' && helpers.isModulePath(property.value.value)) {
                    astTemplate = property;
                    template = property.value.value;
                } else if (property.key.name === 'styles' && helpers.isModulePath(property.value.value)) {
            ...
            helpers.parallel([
                function (done) {
                    ...
                    helpers.readFile(template, done);
                },
                function (done) {
        ...
        .when(patterns.module, function (chunk, done) {
            ...
            helpers.map(chunk.arguments[1].elements, function (element, done) {
                ...
                if (helpers.isLibraryPath(_filepath)) {
                    if (files.hasOwnProperty(_filepath))return done();
                    helpers.readFile(_filepath.slice(2), function (err, content) {
                        if (err)return done(err);
                        emitter.emit('data', new Buffer(content + '\n', 'utf-8'));
                        files[_filepath] = {};
                        done();
                    });
                } else if (helpers.isModulePath(_filepath)) {
                    ...
                    if (files.hasOwnProperty(_filepath)) {
                        ...
                        if (helpers.isDirectory(_filepath)) {
                            helpers.readDirectory(_filepath, module, next)
                        } else {
                            helpers.readFile(_filepath, next);
                        }
                    }
                } else {
    ...
    var newAst = astra(ast)
        .when([
            ...
            if (template === null) {
                ...
                template = helpers.readFileSync(template);
            }
        ...
        .when(patterns.module, function (chunk) {
            ...
            while (i--) {
                ...
                if (helpers.isLibraryPath(_filepath) ) {
                    if(!files.hasOwnProperty(_filepath)) {
                        contents.push(helpers.readFileSync(_filepath.slice(2)));
                        files[_filepath] = {};
                    }
                } else if (helpers.isModulePath(_filepath)) {
                    _filepath = _filepath.substring(1);
