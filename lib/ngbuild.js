        ...
        .when(directiveDesc, function (chunk) {
            ...
            for (var i in funcAst.body.body[0].argument.properties) {
                ...
                if (property.key.name === 'template' && isAllowedPass(property.value.value)) {
                    try {
                        property.value.value = read(property.value.value);
                    } catch (e) {
...
module.exports = function (setting) {
    ...
    if ('undefined' === typeof setting.styles) {
        console.log('ngbuild not building styles now');
    }
    ...
    if ('undefined' !== typeof setting.styles) {
        fs.outputFileSync(setting.styles, build.styles);
    }
